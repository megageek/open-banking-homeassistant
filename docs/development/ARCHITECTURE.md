...
This document describes the technical architecture of the Nordigen for Home Assistant custom component for Home Assistant.

## Directory Structure

```text
custom_components/nordigen/
...
│   ├── __init__.py          # Exports NordigenDataUpdateCoordinator
...
│   ├── __init__.py          # Exports NordigenEntity
...
- `base.py` - Main coordinator class (`NordigenDataUpdateCoordinator`)
...
**Key class:** `NordigenDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `NordigenConfigFlowHandler` (main flow)
- `NordigenOptionsFlow` (options)
...
**Key class:** `NordigenEntity` (in `entity/base.py`)
...
2. `NordigenEntity` for common functionality
...
1. Create directory: `custom_components/nordigen/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `NordigenEntity`
