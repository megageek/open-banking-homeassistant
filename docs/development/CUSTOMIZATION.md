...
Every Monday at 07:00 UTC, the workflow checks whether the upstream blueprint (`megageek/open-banking-homeassistant`) has new commits. If it does, it opens a pull request with the diff against your repository.
...
That's it. No workflow runs, no PRs, no noise. You can still pull upstream changes manually at any time by comparing your repository against `megageek/open-banking-homeassistant`.
