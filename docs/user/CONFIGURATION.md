...
This document describes all configuration options and settings available in the Nordigen for Home Assistant custom integration.
...
```yaml
service: nordigen.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: nordigen.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Nordigen for Home Assistant Alert
...
- [GitHub Issues](https://github.com/megageek/open-banking-homeassistant/issues) - Report problems
