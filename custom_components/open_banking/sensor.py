...
from . import const, logger, DOMAIN
from .ng import get_accounts

pattern = re.compile(r"(?<!^)(?=[A-Z])")

ATTRIBUTION = "Data provided by GoCardless"
...
async def async_setup_entry(hass, entry, async_add_entities):
    requisitions = hass.data[DOMAIN][entry.entry_id].get("requisitions")

    await async_setup_platform(
        hass,
        entry.data,
        async_add_entities,
        discovery_info={"requisitions": requisitions},
    )
...
async def async_setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    logger.info(
        "GoCardless will attempt to configure [%s] requisitions",
        len(discovery_info["requisitions"] or []),
    )
    ...
    logger.info("Total of [%s] GoCardless account sensors configured", len(entities))
...
def balance_update(logger, async_executor, fn, account_id):
    async def update():
        ...
        try:
            ...
            raise UpdateFailed(f"Error updating GoCardless sensors: {err}")
...
def build_coordinator(hass, logger, updater, interval, reference):
    return DataUpdateCoordinator(
        hass,
        logger,
        name=f"open-banking-balance-{reference}",
...
async def build_account_sensors(hass, logger, account, const, device, debug):
    fn = (
        random_balance
        if debug
        else hass.data[const["DOMAIN"]]["client"].account.balances
    )
    ...
    balance_coordinator = build_coordinator(
        ...
        updater=updater,
        interval=interval,
        reference=account.get("unique_ref"),
    )
    ...
    balance_types = get_balance_types(
        logger=logger, config=account["config"], field=const["BALANCE_TYPES"]
    )
...
async def build_sensors(hass, logger, account, const, debug=False):
    return await build_requisition_sensor(
        hass=hass, logger=logger, requisition=account, const=const, debug=debug
    )
    ...
    def state_attributes(self):
        ...
        if self.state:
            ...
            sensor_job = self._setup_account_sensors(
                client=self._client,
                accounts=self.coordinator.data.get("accounts"),
                ignored=self._ignored_accounts,
            )
