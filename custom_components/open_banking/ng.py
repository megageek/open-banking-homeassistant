...
def get_requisitions(client, configs, logger, const):
    ...
    try:
        ...
        logger.error("Unable to fetch GoCardless requisitions: %s", error)
...
def get_or_create_requisition(
    ...
    if requisition.get("status") != "LN":
        ...
        logger.info(
            "Authenticate and accept connection and restart :%s", requisition["link"]
        )
...
def get_account(fn, id, requisition, logger):
    ...
    try:
        ...
        logger.error("Unable to fetch account details from GoCardless: %s", error)
        return
