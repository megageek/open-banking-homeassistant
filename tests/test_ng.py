...
class TestRequisition(unittest.TestCase):
    ...
    @unittest.mock.patch("custom_components.open_banking.ng.matched_requisition")
...
class TestGetAccounts(unittest.TestCase):
    def test_api_exception(self):
        ...
        logger.error.assert_called_with(
            "Unable to fetch GoCardless requisitions: %s", error
        )
    ...
    def test_ignored(self):
        ...
        get_accounts(
            fn=fn, requisition={"accounts": [123]}, logger=logger, ignored=[123]
        )

        logger.info.assert_called_with("Account ignored due to configuration :%s", 123)

    @unittest.mock.patch("custom_components.open_banking.ng.get_account")
