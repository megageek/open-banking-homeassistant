...
from custom_components.open_banking.sensor import (
    BalanceSensor,
    RequisitionSensor,
...
class TestBuildCoordinator(unittest.TestCase):
    def test_basic(self):
        ...
        self.assertEqual(res.name, "open-banking-balance-ref")
...
class TestRequisitionUpdate:
    ...
    async def test_return(self):
        ...
        fn = MagicMock()
        logger = MagicMock()
        res = requisition_update(
            logger=logger, async_executor=executor, fn=fn, requisition_id="id"
        )
        res = await res()
    ...
    async def test_exception(self):
        ...
        logger = MagicMock()
        res = requisition_update(
            logger=logger, async_executor=executor, fn=balance, requisition_id="id"
        )
...
class TestBalanceUpdate:
    ...
    async def test_return(self):
        ...
        logger = MagicMock()
        res = balance_update(
            logger=logger, async_executor=executor, fn=fn, account_id="id"
        )
        res = await res()
    ...
    async def test_exception(self):
        ...
        logger = MagicMock()
        res = balance_update(
            logger=logger, async_executor=executor, fn=balance, account_id="id"
        )
...
class TestBuildSensors:
    @unittest.mock.patch(
        "custom_components.open_banking.sensor.build_requisition_sensor"
    )
    @unittest.mock.patch("custom_components.open_banking.sensor.build_account_sensors")
    @pytest.mark.asyncio
    async def test_build_sensors_unconfirmed(
        self, mocked_build_account_sensors, mocked_build_requisition_sensor
    ):
        args = {
            "hass": "hass",
            "logger": "logger",
...
class TestBuildAccountSensors:
    ...
    @unittest.mock.patch("custom_components.open_banking.sensor.random_balance")
    @unittest.mock.patch("custom_components.open_banking.sensor.build_coordinator")
    @unittest.mock.patch("custom_components.open_banking.sensor.balance_update")
    @pytest.mark.asyncio
    async def test_balance_debug(
        self, mocked_balance_update, mocked_build_coordinator, mocked_random_balance
    ):
        account = {
            "config": {
                "refresh_rate": 1,
    ...
    @unittest.mock.patch("custom_components.open_banking.sensor.BalanceSensor")
    @unittest.mock.patch("custom_components.open_banking.sensor.random_balance")
    @unittest.mock.patch("custom_components.open_banking.sensor.build_coordinator")
    @unittest.mock.patch("custom_components.open_banking.sensor.timedelta")
    @unittest.mock.patch("custom_components.open_banking.sensor.balance_update")
    @pytest.mark.asyncio
...
class TestSensors(unittest.TestCase):
    ...
    @unittest.mock.patch("custom_components.open_banking.sensor.datetime")
...
class TestRequisitionSensor(unittest.TestCase):
    ...
    @unittest.mock.patch("custom_components.open_banking.sensor.get_accounts")
    ...
    @unittest.mock.patch("custom_components.open_banking.sensor.build_account_sensors")
    @unittest.mock.patch("custom_components.open_banking.sensor.datetime")
    def test_unconfirmed_state_attributes_linked(
        self, mocked_datatime, mocked_build_account_sensors
    ):
        mocked_datatime.now.return_value = "last_update"
...
class TestAccountSensorSetup:
    ...
    @unittest.mock.patch("custom_components.open_banking.sensor.datetime")
    @pytest.mark.asyncio
    async def test_setup_account_sensors_new(
        self, mocked_datatime, mocked_build_account_sensors
    ):
        mocked_datatime.now.return_value = "last_update"
        mocked_build_account_sensors.return_value = [
        ...
        await sensor._setup_account_sensors(
            client=mocked_client, accounts=["account-1"], ignored=[]
        )
...
class TestBuildUnconfirmedSensor:
    ...
    @unittest.mock.patch("custom_components.open_banking.sensor.build_coordinator")
    @pytest.mark.asyncio
    async def test_build_requisition_sensor(
        self, mocked_build_coordinator, mocked_timedelta
    ):
        hass = MagicMock()
        logger = MagicMock()
        requisition = {
        ...
        sensors = await build_requisition_sensor(
            hass, logger, requisition, const, False
        )
...
class TestAsyncSetupPlatform:
    @unittest.mock.patch("custom_components.open_banking.sensor.logger")
    @pytest.mark.asyncio
    async def test_no_discovery_info(self, mocked_logger):
        await async_setup_platform(
            hass=None, config=None, add_entities=None, discovery_info=None
        )
    ...
    async def test_no_requisitions(self, mocked_logger):
        ...
        mocked_logger.info.assert_called_with(
            "GoCardless will attempt to configure [%s] requisitions", 0
        )
    ...
    async def test_basic(self, mocked_logger, mocked_build):
        ...
        mocked_logger.info.assert_has_calls(
            [
                call("GoCardless will attempt to configure [%s] requisitions", 2),
                call("Total of [%s] GoCardless account sensors configured", 2),
            ],
        )
